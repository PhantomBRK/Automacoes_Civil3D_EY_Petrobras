...
namespace AutomacoesCivil3D
{
    public static class IfcParametersService
    {
        ...
        private static void SetIfcParamOnEntity(Transaction TransCad, Entity ent, string paramName, string value)
        {
            DBDictionary ifcDict = GetOrCreateIfcSubDictionary(TransCad, ent);
    ...
    public class IfcParametersCommands
    {
        ...
        public void AplicarParametrosIfcNaSelecao()
        {
            ...
            try
            {
                ...
                Transaction TransCad = db.TransactionManager.StartTransaction();
        ...
        // Método auxiliar para não expor internamente o SetIfcParamOnEntity (se quiser manter service mais "fechado")
        private void SetIfcParamSafe(Transaction TransCad, Entity ent, string paramName, string value)
        {
            // Reutiliza a lógica interna do serviço por reflexão simples do mesmo arquivo
            // Aqui, por simplicidade, chamamos diretamente um método privado duplicado (ou torne-o público se preferir).
            // Para manter o contrato de não "inventar" APIs, mantivemos o escopo no mesmo namespace.
            // Implementação direta:
            DBDictionary ifcDict = GetOrCreateIfcSubDictionaryInternal(TransCad, ent);
        ...
        private DBDictionary GetOrCreateIfcSubDictionaryInternal(Transaction TransCad, Entity ent)
        {
            if (ent.ExtensionDictionary.IsNull)
            {
                ent.CreateExtensionDictionary();
            ...
            if (extDict.Contains("IFC"))
            {
                ObjectId ifcDictId = extDict.GetAt("IFC");
                ifcDict = (DBDictionary)TransCad.GetObject(ifcDictId, OpenMode.ForWrite);
            }
            else
            {
                ifcDict = new DBDictionary();
                ObjectId newId = extDict.SetAt("IFC", ifcDict);
                TransCad.AddNewlyCreatedDBObject(ifcDict, true);
            }
