...
{
    public static class QuantitiesIfcSmec_Ifc4
    {
        ...
        [CommandMethod("SIFC_QTO_SMEC_IFC4")]
        public static void SifcQtoSmecIfc4()
        {
            ...
            try
            {
                ...
                docEditor.WriteMessage($"\nOK (IFC4). QTO: {updatedQto} | Meta: {renamed} | TypeFix: {fixedTypes} | Sistemas: {linkedSystems} | Classif: {linkedClassifications}\nSaída: {outputIfcPath}\n");
            }
            catch (Exception ex)
            {
           ...
           XbimEditorCredentials editor = new XbimEditorCredentials
            {
                ...
                ApplicationFullName = "SMEC QTO Injector (IFC4)",
                ApplicationIdentifier = "AutomacoesCivil3D.SMECQTO.IFC4",
            ...
            using IfcStore model = IfcStore.Open(
                inputIfcPath,
                editorDetails: editor,
                accessMode: XbimDBAccess.ReadWrite
            );

            using Xbim.Common.ITransaction tr = model.BeginTransaction("Inject SMEC QTO + Rename TAG + Fix PipeType (IFC4)");

            int updated = 0;
            int renamed = 0;

            // 1) QTO para Chambers (IFC4)
        ...
        private static int ApplyDrainageSystems(IfcStore model)
        {
            int linked = 0;
        ...
        private static IfcSystem GetOrCreateSystem(IfcStore model, string systemName, string system, string subsystem)
        {
        ...
        private static bool EnsureAssignedToSystem(IfcStore model, IIfcObject obj, IfcSystem ifcSystem)
        {
        ...
        private static IfcElementQuantity GetOrCreateElementQuantity(IfcStore model, IIfcObject obj, string qtoName)
        {
            IIfcRelDefinesByProperties[] rels = obj.IsDefinedBy
                .OfType<IIfcRelDefinesByProperties>()
                .ToArray();
        ...
        private static int ApplyPipeSegmentPredefinedTypeFromMetadata(IfcStore model)
        {
            int changed = 0;
        ...
        private static int ApplyDistributionChamberPredefinedTypeFromMetadata(IfcStore model)
        {
            int changed = 0;
        ...
        private static int ApplyPipeFittingPredefinedTypeFromMetadata(IfcStore model)
        {
            int changed = 0;
        ...
        private static int ApplyDrainageClassification(IfcStore model)
        {
            int linked = 0;
        ...
        private static bool EnsureClassificationAssociation(IfcStore model, IIfcObject obj, string identification, string name)
        {
        ...
        private static bool HasClassificationAssociation(IfcStore model, IIfcObject obj, string identification)
        {
        ...
        private static object GetOrCreateClassificationReference(IfcStore model, string identification, string name)
        {
        ...
        private static object GetOrCreateClassification(IfcStore model)
        {
        ...
        private static object CreateSchemaEntity(IfcStore model, string suffix)
        {
        ...
        private static string ResolveSchemaRoot(IfcStore model)
        {
