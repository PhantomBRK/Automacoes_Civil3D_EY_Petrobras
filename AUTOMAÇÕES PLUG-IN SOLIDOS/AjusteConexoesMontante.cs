...
{
    public class SolidosConectarRedePorMontante
    {
       ...
       private static readonly string[] CotaSaidaParamCandidates = new string[]
        ...
        [CommandMethod("SOL_CONECTAR_REDE_POR_MONTANTE", CommandFlags.Modal)]
        public void ConectarRedePorMontanteMantendoDeclividade()
        {
            ...
            try
            {
                ObjectId upId = GetNodeIdBySelection(docEditor, "\nSelecione o DISPOSITIVO A MONTANTE (âncora): ");
                if (upId.IsNull)
                {
                ...
                // Z de referência do dispositivo âncora: Location.Z (normal) OU cotaSaida (RALO/FUNIL)
                double upZ = GetDeviceOutletZ(upId);
                ToggleLadoDerivaIfPresent(upId, ladoDerivaOriginalByDevice);
                CaptureInitialOutletZIfNeeded(upId, initialOutletZByDevice, upZ);
                ...
                progress.Start("Processando rede SOLIDOS (montante → jusante)...");
                ...
                while (queue.Count > 0)
                {
                    ...
                    ObjectId curUpDevice = queue.Dequeue();
                    inQueue.Remove(curUpDevice);
                    ToggleLadoDerivaIfPresent(curUpDevice, ladoDerivaOriginalByDevice);
                    CaptureInitialOutletZIfNeeded(curUpDevice, initialOutletZByDevice);

                    double curUpZ;
                    if (!zByDevice.TryGetValue(curUpDevice, out curUpZ))
                    {
                        curUpZ = GetDeviceOutletZ(curUpDevice);
                        zByDevice[curUpDevice] = curUpZ;
                    }

                    List<ObjectId> connected = TryGetConnectedDevices(curUpDevice);
                    foreach (ObjectId pipeId in connected)
                    {
                        ...
                        // Tubos que "saem" do dispositivo atual (montante do tubo)
                        if (pipeInfo.InPart.IsNull || pipeInfo.InPart != curUpDevice)
                        {
                            continue;
                        }

                        ObjectId downDevice = pipeInfo.OutPart;
                        if (downDevice.IsNull)
                        {
                            continue;
                        }

                        ToggleLadoDerivaIfPresent(downDevice, ladoDerivaOriginalByDevice);
                        CaptureInitialOutletZIfNeeded(downDevice, initialOutletZByDevice);

                        // Ajusta o tubo para bater no Z do dispositivo montante, mantendo declividade
                        bool changed = AdjustPipeToUpstreamZ(pipeInfo, curUpDevice, curUpZ, out double newDownZ);
                        if (changed)
                        {
                            adjustedPipes++;
                        }

                        // Ajusta "saída" do dispositivo jusante:
                        // - normal: Location.Z
                        // - RALO/FUNIL: cotaSaida
                        bool devChanged = SetDeviceOutletZOnly(downDevice, newDownZ);
                        if (devChanged)
                        {
                            adjustedDevices++;
                        }

                        bool hadTarget = zByDevice.TryGetValue(downDevice, out double oldDownZ);
                        bool targetChanged = (!hadTarget) || (Math.Abs(oldDownZ - newDownZ) > ZTol);
                        zByDevice[downDevice] = newDownZ;

                        if (targetChanged && !inQueue.Contains(downDevice))
                        {
                            queue.Enqueue(downDevice);
                            inQueue.Add(downDevice);
                        }
                    }
                }
                ...
                docEditor.WriteMessage(
                    $"\nOK. Ajuste por montante concluído. Tubos ajustados: {adjustedPipes}. Dispositivos (Z saída) ajustados: {adjustedDevices}.");
            }
        ...
        private static bool AdjustPipeToUpstreamZ(PipeInfo pipe, ObjectId upDevice, double upZ, out double downZ)
        {
            downZ = 0.0;
            ...
            GeometryPoint upLoc = TryGetParam<GeometryPoint>(upDevice, ParamLocation);
            EndpointSide sideAtUp = ResolveEndpointSide(pipe, upDevice, upLoc);
            bool upAtStart = sideAtUp == EndpointSide.StartPoint;

            GeometryPoint upPt = upAtStart ? sp : ep;
            GeometryPoint downPt = upAtStart ? ep : sp;
            ...
            double newUpZ = upZ;
            double newDownZ = newUpZ + (slope * planLen);

            if (Math.Abs(upPt.Z - newUpZ) <= ZTol && Math.Abs(downPt.Z - newDownZ) <= ZTol)
            {
                downZ = downPt.Z;
                return false;
            }
        ...
        private static bool IsRaloOrFunil(ObjectId devId)
        {
            ...
            return u.Contains("RALOS") || u.Contains("FUNIL");
        }
